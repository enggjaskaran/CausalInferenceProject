...
ate_df_p4_ow <- data.frame()
se_df_p4_ow <- data.frame()
...
for(j in 1:length(p_array)){
  ...
  for (i in 1:length(n_array)){
    ...
    ate_df_p4_ow <- rbind(ate_df_p4_ow,output[[1]] %>% mutate(n=n,p=p))
    se_df_p4_ow <- rbind(se_df_p4_ow,output[[2]] %>% mutate(n=n,p=p))
  }
}  



ate_df_p4 %>% filter(p==4) %>% 
